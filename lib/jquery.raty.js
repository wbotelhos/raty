  ...
  var methods = {
    init: function(settings) {
      return this.each(function() {
        ...
        self.opt.number = methods._between(self.opt.number, 0, self.opt.numberMax)
        ...
        if (self.opt.cancel) {
          self.cancel = methods._createCancel.call(this);

          width += (self.opt.size + space);
        }

        if (self.opt.readOnly) {
          methods._lock.call(self);
        } else {
          that.css('cursor', 'pointer');

          methods._binds.call(self);
        }
        ...
        that.data({ 'settings': self.opt, 'raty': true });
      });
      ...
      self.cancel.on('mouseenter.raty', function(evt) {
      ...
      return this.each(function() {
        ...
        if (that.data('readonly') === isReadOnly) {
          return this;
        }
      ...
      this.each(function() {
        var that  = $(this),
          ...
          clone  = that.clone().removeAttr('style').insertBefore(that);
