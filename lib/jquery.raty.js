  ...
  var methods = {
    ...
    _adjustStarType: function() {
      ...
      for (var i = 0; i < replaces.length; i++) {
        element.opt[replaces[i]] = element.opt[replaces[i]].replace('.', '-');
      }
    },
    ...
    _createStars: function() {
      ...
      for (var i = 1; i <= this.opt.number; i++) {
        var
          ...
          attributes = { src: icon, alt: i },
          star       = $('<' + this.opt.starType + ' />', { title: title });

        icon = this.opt.path + this.opt[icon];

        if (this.opt.starType !== 'img'){
          attributes = { 'data-alt': i, 'class': icon };
        }

        star.attr(attributes).appendTo(this);

        if (this.opt.space) {
          that.append((i < this.opt.number) ? '&#160;' : '');
        }
      }
      return that.children(this.opt.starType);
      ...
      for (var i = 1; i <= self.stars.length; i++) {
        ...
        if (self.opt.iconRange && self.opt.iconRange.length > hash) {
          var irange = self.opt.iconRange[hash],
              ...
              icon   = this.opt.path + (select ? on : off);
          ...
          var icon = this.opt.path + this.opt[select ? 'starOn' : 'starOff'];
