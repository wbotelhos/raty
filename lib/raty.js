class Raty {
  ...
  constructor(element, options = {}) {
    ...
    // TEMP: conditional to migrate element to a query selected object over just a string selector
    this.self = typeof element === 'string' ? document.querySelector(element) : element;
    this.opt = { ...this.defaultOptions(), ...options, ...this._parseOptions(this.self.dataset) };
  }
  ...
  cancel(click) {
    if (!this._isTrue(this.self.dataset.readOnly)) {
  ...
  move(score) {
    ...
    evt.pageX = percent;
    this.self.isMove = true;
    star.dispatchEvent(evt);
    this.self.isMove = false;
  }

  // TODO: model spec
  readOnly(readonly) {
    if (this._isTrue(this.self.dataset.readOnly) !== readonly) {
      if (readonly) {
        this._lock();
      } else {
        this._binds();
  ...
  _bindClick() {
    ...
    this.stars.forEach((value) => {
      value.addEventListener('click', function (evt) {
        if (that._isTrue(that.self.dataset.readOnly)) {
          return;
        }
        var execute = true;
  ...
  _createStars() {
    ...
    this.stars = this.self.querySelectorAll(this.opt.starType);
  }
  ...
  _executeCallbacks() {
    ...
    for (let i = 0; i < options.length; i++) {
      if (typeof this.opt[options[i]] === 'function') {
        const value = this.opt[options[i]].call(this.self);
