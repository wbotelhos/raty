class Raty {
  ...
  _createStars() {
    ...
    let stars = document.querySelectorAll(`${this.element} > ${this.opt.starType}`);
    this.stars = stars;
  }
  ...
  _parseOptions(dataset) {
    return Object.keys(dataset).reduce((acc, key) => {
      const value = { true: true, false: false }[dataset[key]];

      acc[key] = value !== null && value !== undefined ? value : dataset[key];
