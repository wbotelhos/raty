class Raty {
  ...
  cancel(click) {
    if (!this._isTrue(this.element.dataset.readOnly)) {
  ...
  move(score) {
    ...
    evt.pageX = percent;
    this.element.isMove = true;
    star.dispatchEvent(evt);
    this.element.isMove = false;
  }

  // TODO: model spec
  readOnly(readonly) {
    if (this._isTrue(this.element.dataset.readOnly) !== readonly) {
      if (readonly) {
        this._lock();
      } else {
        this._binds();
  ...
  _bindClick() {
    ...
    this.stars.forEach((value) => {
      value.addEventListener('click', function (evt) {
        if (that._isTrue(that.element.dataset.readOnly)) {
          return;
        }
        var execute = true;
        var score = that.opt.half || that.opt.precision ? that.element.dataset.score : value.alt || value.dataset.alt;
        ...
        if (that.opt.click) {
          execute = that.opt.click.call(that, +score, that.element, evt);
        }
  ...
  _bindOutCancel() {
    ...
    this.cancelButton.addEventListener('mouseleave', function (evt) {
      ...
      if (that.opt.mouseout) {
        ...
        that.opt.mouseout.call(that, score, that.element, evt);
      }
    });
  }
    ...
    this.stars.forEach((value) => {
      value.addEventListener(action, function (evt) {
        ...
        if (that.opt.half) {
          ...
          that.element.dataset.score = score;
        }

        that._target(score, evt);

        if (that.opt.mouseover) {
          that.opt.mouseover.call(that, score, that.element, evt);
        }
      });
    });
  }
  ...
  _bindOverCancel() {
    ...
    this.cancelButton.addEventListener('mouseover', function (evt) {
      ...
      if (that.opt.mouseover) {
        that.opt.mouseover.call(that, null, that.element, evt);
      }
    });
  }
  ...
  _createStars() {
    for (var i = 1; i <= this.opt.number; i++) {
      ...
      let el = document.createElement(this.opt.starType);
  ...
  _executeCallbacks() {
    ...
    for (let i = 0; i < options.length; i++) {
      if (typeof this.opt[options[i]] === 'function') {
        const value = this.opt[options[i]].call(this.element);
  ...
  _getHint(score, evt) {
    ...
    var hint = group;
    var set = !evt || this.element.isMove;
  ...
  _roundStars(score, evt) {
    ...
    if (name) {
      let icon = this.opt[name];
      let star = this.stars[Math.ceil(score) - 1];
  ...
  _setPath() {
    ...
    // TODO: do not need the [0] that will always return the first letter of the string.
